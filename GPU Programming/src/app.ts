	...
	updateScene(time: DOMHighResTimeStamp){
        ...
        switch (this.programNumber) {
          case 0:
            ...
            if (num!.valueAsNumber < 0.8){
            this.uniforms[0].u_ambient.value.setScalar((1- num!.valueAsNumber))
            }
            break;

          case 1:
            this.uniforms[1].u_color.value.offsetHSL( 1.0, 0, 0 );
            if (num!.valueAsNumber == 0.0){
                num!.valueAsNumber = 0.1;
            }
            this.uniforms[1].u_ambient.value.setScalar(num!.valueAsNumber)
            break;

          case 2:
            this.uniforms[2].u_color.value.offsetHSL( 1.0, 0, 0 );
            //var t = num!.valueAsNumber
            if (num!.valueAsNumber == 0.0){
                num!.valueAsNumber = 0.1;
            }
            /*for (t == 0.0; t<1.0; t++) {
