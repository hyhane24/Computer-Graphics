...
float circle(in vec2 d, in float rad, float f){
    vec2 distan = d-vec2(f);
	return smoothstep(rad-(rad*0.01),rad+(rad*0.01),dot(distan,distan)*6.0);
}
...
void main() {
    ...
    vec2 v = v_texcoord.xy/u_ambient.xy;;

    vec3 color = vec3(circle(v,0.1, 0.3));
