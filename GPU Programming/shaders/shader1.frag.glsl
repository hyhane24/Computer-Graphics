...
void main() {
    ...
    //vec4 gray = vec4( vec3( tcolor.r * 0.3 + tcolor.g * 0.59 + tcolor.b * 0.11 ), 1.0 );
     ...
     vec2 vu = v_texcoord.xy/u_ambient.xy;
		
     float i = 2.5;

     float xx = step(0.5, mod(vu.x * i, 1.0));
     xx *= step(0.5, mod(vu.y * i, 1.0));
     float yy = step(0.5, mod(vu.x * i + 0.5, 1.0));
     yy *= step(0.5, mod(vu.y * i + 0.5, 1.0));
    ...
    gl_FragColor = vec4(xy, xy, xy, 1.0);
}
